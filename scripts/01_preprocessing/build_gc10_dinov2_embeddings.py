"""Build frozen DINOv2 embeddings for the opaque GC10 acquisition pool."""
...
DEFAULT_OUT = ROOT / "outputs" / "gc10_visual_embeddings" / "dinov2_small_protocol_20260715"
EXPECTED_ROWS = 1836
...
def main() -> None:
    ...
    config = json.loads((protocol / "gc10_protocol_config.json").read_text(encoding="utf-8"))
    if bool(config.get("final_test_evaluated", True)):
        raise RuntimeError("Protocol does not certify final_test_evaluated=False")
    blind_path = protocol / "gc10_acquisition_pool_blind.csv"
    loader_path = protocol / "gc10_acquisition_loader_private.csv"
    blind = pd.read_csv(blind_path)
    if blind.columns.tolist() != EXPECTED_BLIND_COLUMNS:
        raise RuntimeError(f"Unexpected blind columns: {blind.columns.tolist()}")
    loader = pd.read_csv(loader_path)
    if loader.columns.tolist() != ["sample_id", "image_path"]:
        raise RuntimeError(f"Unexpected loader columns: {loader.columns.tolist()}")
    if len(blind) != EXPECTED_ROWS or blind["sample_id"].duplicated().any():
        raise RuntimeError(f"Expected {EXPECTED_ROWS} unique blind rows, got {len(blind)}")
    ...
    config_path = out / "embedding_config.json"
    if config_path.exists() and (out / "embeddings.npy").exists() and (out / "embedding_manifest.csv").exists():
    ...
    if embeddings.shape != (EXPECTED_ROWS, 384) or not np.isfinite(embeddings).all():
        raise RuntimeError(f"Unexpected embedding output: {embeddings.shape}")
    ...
    public = manifest[["embedding_index", "sample_id", "image_sha256"]].copy()
    np.save(out / "embeddings.npy", embeddings.astype(np.float32))
    public.to_csv(out / "embedding_manifest.csv", index=False, encoding="utf-8-sig")
    result = {
        "status": "success",
        "backend": "dinov2",
        "model_id": "facebook/dinov2-small",
        "blind_manifest_sha256": blind_hash,
        "num_embeddings": len(public),
        "embedding_shape": list(embeddings.shape),
        "source_paths_exported": False,
        "uses_gt_labels": False,
        "uses_xml_or_bboxes": False,
    ...
    print("=" * 100)
    print("[DONE] GC10 frozen DINOv2 embeddings")
    print("GT/XML/bbox used: False")
