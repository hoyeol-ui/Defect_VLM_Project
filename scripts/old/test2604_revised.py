...
EXPERIMENT_NAME = "consistency_v3_rephrase_viewpoint_sbert_first"
...
# BERTScore는 매우 느려서 low/high 후보에 대해서만 후처리
USE_BERTSCORE_FULL = False
USE_BERTSCORE_CANDIDATES = True
...
METHOD_UPSTREAM_SOURCES = {
    "selfcheckgpt": {
        ...
        "adaptation_note": (
            "SelfCheckGPT uses multiple generations to estimate self-consistency. "
            "This experiment adapts that idea to VLM defect descriptions."
        ),
    },
    "semantic_uncertainty": {
        "official_repo": "https://github.com/lorenzkuhn/semantic_uncertainty",
        "adaptation_note": (
            "Semantic uncertainty focuses on meaning-level uncertainty rather than surface-form variation. "
            "This experiment adapts that idea by comparing generated defect descriptions semantically."
        ),
    },
    "badge": {
        "official_repo": "https://github.com/JordanAsh/badge",
        "adaptation_note": (
            "BADGE is cited as active-learning method lineage. "
            "This script prepares candidate sets but does not reproduce BADGE gradient embeddings."
        ),
    },
    "coreset": {
        "official_repo": "https://github.com/ozansener/active_learning_coreset",
        "adaptation_note": (
            "Core-Set is cited as diversity-based active-learning lineage. "
            "This script prepares candidate sets but does not reproduce Core-Set optimization."
        ),
    },
}
...
# Rationale:
# - 실험 결과, 요약, plot, manifest를 구조화해 재현성 확보.
# -----------------------------------------------------------------------------
...
# Reference:
# - Qwen2-VL model card
# - Qwen2-VL Transformers docs
# - SentenceTransformer API docs
#
# Rationale:
# - Qwen2-VL + SBERT 기반 VLM explanation consistency 실험 환경 구성.
# -----------------------------------------------------------------------------
def load_models():
...
# -----------------------------------------------------------------------------
# Reference:
# - Qwen2-VL official example flow:
#   apply_chat_template -> process_vision_info -> processor(...) -> model.generate(...)
# - process_vision_info upstream source
# - SelfCheckGPT official repository
#
# Adaptation note:
# - SelfCheckGPT의 multi-generation consistency 개념을 VLM defect description에 적용.
# - 동일 이미지에 여러 prompt를 적용해 복수 설명을 생성.
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Reference:
# - SBERT embedding 간 cosine similarity 계산.
# -----------------------------------------------------------------------------
...
# - Semantic Uncertainty repository
#
# Adaptation note:
# - SelfCheckGPT: 복수 응답 간 self-consistency 측정 아이디어 적용.
# - Semantic Uncertainty: 표면 표현보다 의미 수준 비교가 중요하다는 관점 적용.
# - 본 함수는 semantic entropy 원본 구현이 아니라 SBERT cosine 기반 proxy 구현.
# -----------------------------------------------------------------------------
...
#
# Rationale:
# - 피드백 3번: SBERT 외 비교 가능한 지표 추가.
#
# Important:
# - 매우 느리므로(3시간 소요) 전체 60장 × 2조건에 직접 쓰지 말고 후보군 후처리용으로 사용.
# -----------------------------------------------------------------------------
...
# - SelfCheckGPT repository
# - Semantic Uncertainty repository
# - Python JSONL logging pattern

# - 이미지별 복수 응답, prompt group, pairwise score, generation config를 보존.
# -----------------------------------------------------------------------------
    ...
    with open(output_jsonl_path, "w", encoding="utf-8") as f:
        for idx, image_path in enumerate(image_paths, start=1):
            ...
            record = {
                ...
                "method_adaptation_note": (
                    "Multi-response self-consistency and semantic-level uncertainty ideas "
                    "are adapted to VLM-based defect-description analysis."
                ),
...
# -----------------------------------------------------------------------------
# Reference:
# - statistics module docs
#
# Rationale:
# -피드백 1번: 전체 분포, 클래스별 평균/분산 확인.
# -----------------------------------------------------------------------------
...
def plot_distributions(df: pd.DataFrame, metric_key: str, output_dir: Path) -> None:
    ...
    # classwise boxplot using pandas/matplotlib only
...
# Reference:
# - BADGE official repository
# - Core-Set official repository

# - BADGE/Core-Set는 미구현, 결과 확인 후 추가 구현 예정
# - 후속 AL 검증을 위해 low/high/random candidate set 준비 단계.
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Reference:
# - BERTScore repository
#
# Rationale:
# - 전체가 아니라 후보군에 대해서만 BERTScore 후처리(소요시간 줄이기 위해).
# -----------------------------------------------------------------------------
def posthoc_bertscore_for_candidates(candidate_records: List[Dict], output_path: Path) -> List[Dict]:
    ...
    if bertscore_score is None:
        print("BERTScore package not installed. Skipping candidate BERTScore.")
        return candidate_records
    ...
    for idx, rec in enumerate(candidate_records, start=1):
        ...
        print(
            f"[posthoc BERTScore] {idx}/{len(candidate_records)} | "
            f"{rec['prompt_group']} | {rec['class_name']} | "
            f"{new_rec['posthoc_bertscore_mean_score']}"
        )
...
def save_source_manifest(output_path: Path, gen_cfg: GenerationConfig, sampled_images: List[Path]) -> None:
    manifest = {
        ...
        "code_statement": (
            "This script is an experiment-specific adaptation based on official upstream "
            "model cards, API docs, and method-lineage repositories. It is not a verbatim copy."
        ),
        "generation_config": asdict(gen_cfg),
...
def main():
    ...
    # candidate manifests: 조건별로 따로 저장
    rephrase_candidates = save_candidate_manifests(
        rephrase_records,
        "sbert_mean_score",
        dirs["candidates"],
        prefix="rephrase",
    )
    viewpoint_candidates = save_candidate_manifests(
        viewpoint_records,
        "sbert_mean_score",
        dirs["candidates"],
        prefix="viewpoint",
    ...
    # 후보군만 BERTScore 후처리
    candidate_pool = (
        rephrase_candidates["low"]
        + rephrase_candidates["high"]
        + viewpoint_candidates["low"]
        + viewpoint_candidates["high"]
    )

    posthoc_bertscore_for_candidates(
        candidate_pool,
        dirs["summary"] / "posthoc_bertscore_candidates.json",
    )
    ...
    final_compare = {
        ...
        "interpretation_note": [
            "Rephrase condition checks stability under semantically equivalent prompt variation.",
            "Viewpoint condition checks sensitivity under observation-perspective variation.",
            "Low-consistency samples in rephrase are stronger uncertainty candidates.",
            "Low-consistency samples shared across rephrase and viewpoint should be prioritized for human inspection.",
            "BERTScore is computed only for candidates to avoid excessive runtime.",
        ],
    }
